...
public class RewindManager : MonoBehaviour
{
    ...
    private float previousRewindSeconds = 0f;
    ...
    // 카메라의 Superliminal코드 가져오기
    public GameObject cameraGameObject;
    public Superliminal superliminal;

    private void Start()
    {
        superliminal = cameraGameObject.GetComponent<Superliminal>();
    }
    ...
    {   
        if (IsBeingRewinded)
        {
            ...
            float roundedRewindSeconds = Mathf.Round(rewindSeconds * 10000f) / 10000f;
            float roundedPreviousRewindSeconds = Mathf.Round(previousRewindSeconds * 10000f) / 10000f;

            Debug.Log($"rewindSeconds: {rewindSeconds}, previousRewindSeconds: {previousRewindSeconds}");
            Debug.Log($"roundedRewindSeconds: {roundedRewindSeconds}, roundedPreviousRewindSeconds: {roundedPreviousRewindSeconds}");


            if (roundedRewindSeconds == roundedPreviousRewindSeconds)
            {
                superliminal.usedObjects.Clear();
                Debug.Log("초기화");
            }
            previousRewindSeconds = rewindSeconds;

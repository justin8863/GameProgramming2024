...
public class RewindByKeyPress : MonoBehaviour
{
    ...
    [SerializeField] float rewindIntensity = 0.05f;          //Variable to change rewind speed
    [SerializeField] AudioSource rewindSound;
    float rewindValue = 0;

    void FixedUpdate()
    {
        if(Input.GetKey(KeyCode.T))                     //Change keycode for your own custom key if you want
        {
